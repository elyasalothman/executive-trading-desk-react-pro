...
function PerformanceChart({ trades, theme }) {
  ...
  if (!trades.length) {
    return <div className="empty-state">أضف صفقات إلى السجل لعرض الرسم البياني الشهري للأداء.</div>;
  }
...
function App() {
  ...
  const filteredTrades = useMemo(() => {
    return [...journal].reverse().filter((trade) => {
      const textMatch = !searchText || [trade.symbol, trade.notes, trade.date]
        .join(' ')
        .toLowerCase()
  ...
  function addTrade() {
    ...
    setTradeForm((prev) => ({ ...defaultTradeForm, date: new Date().toISOString().slice(0, 10), symbol: symbolTitle }));
        ...
        <section className="auth-screen">
          <div className="auth-card">
            ...
            <p className="auth-subtitle">
              نسخة React احترافية ومنظمة على ملفات منفصلة، مع سجل صفقات، فلترة وبحث، إحصائيات شهرية،
              رسم بياني للأداء، وإدارة مخاطر متقدمة.
            </p>
    ...
    <>
      ...
      <main className="app-shell">
        ...
        <section className="alert">
          ...
          التركيز على مناطق الارتداد، مراقبة سيولة السلع، والانتباه للأحداث عالية التأثير، مع الالتزام الصارم بإدارة المخاطر قبل أي قرار تداول.
        </section>
        ...
        <section className="stats-grid">
          ...
          <div className="stat-card">
            ...
            <div className={`stat-value ${stats.totalPnl >= 0 ? 'green' : 'red'}`}>${fmt(stats.totalPnl, 2)}</div>
          </div>
          <div className="stat-card">
            <div className="stat-title">متوسط ربح الصفقة</div>
            <div className={`stat-value ${stats.avgPnl >= 0 ? 'green' : 'red'}`}>${fmt(stats.avgPnl, 2)}</div>
          </div>
        </section>
              ...
              <div className="panel-body">
                <div className="grid-2">
                  <div className="field">
                    ...
                    <input id="tradeDate" type="date" value={tradeForm.date} onChange={(e) => handleTradeInput('date', e.target.value)} />
                  </div>
                  <div className="field">
                    <label htmlFor="tradeSymbol">الأصل</label>
                    <input id="tradeSymbol" type="text" value={tradeForm.symbol} onChange={(e) => handleTradeInput('symbol', e.target.value)} placeholder="مثال: SPY أو XAUUSD" />
                  </div>
                </div>
                <div className="grid-2">
                  <div className="field">
                    <label htmlFor="tradeDirection">نوع الصفقة</label>
                    <select id="tradeDirection" value={tradeForm.direction} onChange={(e) => handleTradeInput('direction', e.target.value)}>
                  ...
                  <div className="field">
                    ...
                    <input id="tradeQty" type="number" min="0" step="0.01" value={tradeForm.qty} onChange={(e) => handleTradeInput('qty', e.target.value)} placeholder="مثال: 10" />
                  </div>
                </div>
                <div className="grid-2">
                  <div className="field">
                    <label htmlFor="tradeEntry">سعر الدخول</label>
                    <input id="tradeEntry" type="number" min="0" step="0.0001" value={tradeForm.entry} onChange={(e) => handleTradeInput('entry', e.target.value)} placeholder="مثال: 520.5" />
                  </div>
                  <div className="field">
                    <label htmlFor="tradeExit">سعر الخروج</label>
                    <input id="tradeExit" type="number" min="0" step="0.0001" value={tradeForm.exit} onChange={(e) => handleTradeInput('exit', e.target.value)} placeholder="مثال: 528" />
                  </div>
                </div>
                <div className="field">
                  <label htmlFor="tradeNotes">ملاحظات</label>
                  <textarea id="tradeNotes" value={tradeForm.notes} onChange={(e) => handleTradeInput('notes', e.target.value)} placeholder="سبب الدخول، الملاحظة الفنية، الخطأ أو الدرس المستفاد..."></textarea>
                </div>
                ...
                <div className="panel" style={{ marginTop: '16px', boxShadow: 'none' }}>
                  <div className="panel-body">
                    <div className="toolbar-row" style={{ marginBottom: '12px' }}>
                      ...
                      />
                      <select value={directionFilter} onChange={(e) => setDirectionFilter(e.target.value)} style={{ flex: '1 1 180px' }}>
                        <option value="all">كل الاتجاهات</option>
                        <option value="long">شراء فقط</option>
                        <option value="short">بيع فقط</option>
                      </select>
                      <select value={resultFilter} onChange={(e) => setResultFilter(e.target.value)} style={{ flex: '1 1 180px' }}>
                      ...
                      <select value={monthFilter} onChange={(e) => setMonthFilter(e.target.value)} style={{ flex: '1 1 180px' }}>
                      ...
                      <table>
                        ...
                        <tbody>
                          {filteredTrades.length === 0 ? (
                            <tr>
                              <td colSpan="9" className="empty-state">لا توجد نتائج مطابقة للفلاتر الحالية</td>
                            </tr>
                            ...
                            <tr key={trade.id}>
                              ...
                              <td>{trade.symbol}</td>
                              <td>
                                <span className={`tag ${trade.direction}`}>{trade.direction === 'long' ? 'شراء' : 'بيع'}</span>
                              </td>
                              <td>{fmt(trade.entry, 4)}</td>
                              <td>{fmt(trade.exit, 4)}</td>
                              <td>{fmt(trade.qty, 2)}</td>
                              <td className={trade.pnl >= 0 ? 'pnl-positive' : 'pnl-negative'}>${fmt(trade.pnl, 2)}</td>
                ...
                <div className="monthly-grid">
                  {monthlyStats.length === 0 ? (
                    ...
                    <div className="month-card" key={month.month}>
                      <h4>{month.month}</h4>
                      <p>
                        الصفقات: <strong>{fmt(month.trades, 0)}</strong><br />
                        الرابحة: <strong>{fmt(month.wins, 0)}</strong><br />
                        الخاسرة: <strong>{fmt(month.losses, 0)}</strong><br />
                        الصافي: <strong className={month.totalPnl >= 0 ? 'text-green' : 'text-red'}>${fmt(month.totalPnl, 2)}</strong>
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="panel-body">
                <div className="grid-2">
                  <div className="field">
                    ...
                    <input id="capital" type="number" min="0" step="0.01" value={riskForm.capital} onChange={(e) => handleRiskInput('capital', e.target.value)} placeholder="مثال: 10000" />
                  </div>
                  <div className="field">
                    <label htmlFor="riskPct">نسبة المخاطرة (%)</label>
                    <input id="riskPct" type="number" min="0" step="0.1" value={riskForm.riskPct} onChange={(e) => handleRiskInput('riskPct', e.target.value)} placeholder="1" />
                  </div>
                </div>
                <div className="grid-2">
                  <div className="field">
                    <label htmlFor="entryPx">سعر الدخول</label>
                    <input id="entryPx" type="number" min="0" step="0.0001" value={riskForm.entryPx} onChange={(e) => handleRiskInput('entryPx', e.target.value)} placeholder="مثال: 520.5" />
                  </div>
                  <div className="field">
                    <label htmlFor="stopPx">وقف الخسارة</label>
                    <input id="stopPx" type="number" min="0" step="0.0001" value={riskForm.stopPx} onChange={(e) => handleRiskInput('stopPx', e.target.value)} placeholder="مثال: 515" />
                  </div>
                </div>
                <div className="grid-2">
                  <div className="field">
                    <label htmlFor="targetPx">الهدف السعري (اختياري)</label>
                    <input id="targetPx" type="number" min="0" step="0.0001" value={riskForm.targetPx} onChange={(e) => handleRiskInput('targetPx', e.target.value)} placeholder="مثال: 532" />
                  </div>
                  <div className="field">
                    <label htmlFor="direction">نوع الصفقة</label>
                    <select id="direction" value={riskForm.direction} onChange={(e) => handleRiskInput('direction', e.target.value)}>
                      <option value="long">شراء (Long)</option>
                      <option value="short">بيع (Short)</option>
                    </select>
                  </div>
                ...
                <div className="mini-actions">
                  {[0.5, 1, 2].map((value) => (
                    <button key={value} className="tiny-btn" onClick={() => handleRiskInput('riskPct', String(value))}>{value}%</button>
                  ))}
                  <button className="tiny-btn" onClick={copyCurrentSymbolToTrade}>نسخ الأصل الحالي إلى السجل</button>
                  <button className="tiny-btn" onClick={clearRiskForm}>مسح</button>
                </div>
                <div style={{ marginTop: '14px' }}>
                  <button className="btn btn-primary" style={{ width: '100%', minWidth: 'auto' }} onClick={handleCalculateRisk}>حساب الصفقة</button>
                </div>
                <div className="risk-summary">
                  <div className="metric">
                    <div className="metric-label">المبلغ المعرض للخطر</div>
                    <div className="metric-value text-accent">{riskResult.riskAmount !== null ? `$${fmt(riskResult.riskAmount, 2)}` : '—'}</div>
                  </div>
                  <div className="metric">
                    <div className="metric-label">المخاطرة لكل وحدة</div>
                    <div className="metric-value text-red">{riskResult.riskPerUnit !== null ? `$${fmt(riskResult.riskPerUnit, 4)}` : '—'}</div>
                  </div>
                  <div className="metric">
                    <div className="metric-label">الكمية المسموحة</div>
                    <div className="metric-value text-green">{riskResult.positionSize !== null ? `${fmt(riskResult.positionSize, 2)} وحدة` : '—'}</div>
                  </div>
                  <div className="metric">
                    <div className="metric-label">العائد / المخاطرة</div>
                    <div className="metric-value text-accent">{riskResult.rr !== null ? `${fmt(riskResult.rr, 2)} R` : '—'}</div>
                  </div>
                </div>
        ...
        <div className="footer-note">
          تم تنفيذ كل المطلوب: فصل الملفات، React، فلترة وبحث، إحصائيات شهرية، رسم أداء، تصدير CSV، وحفظ محلي كامل للإعدادات والبيانات.
        </div>
      </main>
    </>
  );
